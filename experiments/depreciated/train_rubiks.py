...
def makeTrainer(
    *,
    ...
    lr=1e-3,
    device="cuda",
    net_config={"num_layers": 3, "ch": rep, "group": RubiksCube2x2()},
    log_level="info",
    trainer_config={"log_dir": None, "log_args": {"minPeriod": 0.02, "timeFrac": 50}},
    save=False
):
    levels = {
    ...
    with FixedNumpySeed(seed), FixedPytorchSeed(seed):
        datasets = {
            "train": BrokenRubiksCube2x2(train=False),
            "test": BrokenRubiksCube2x2(train=False),
        }
    model = Standardize(
        network(datasets["train"].rep_in, datasets["train"].rep_out, **net_config),
        datasets["train"].stats,
    )
    dataloaders = {
        k: LoaderTo(
            DataLoader(
