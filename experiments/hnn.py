...
from trainer.hamiltonian_dynamics import (
    IntegratedDynamicsTrainer,
    DoubleSpringPendulum,
    hnn_trial,
)
...
def makeTrainer(
    *,
    ...
    lr=3e-3,
    device="cuda",
    split={"train": 500, "val": 0.1, "test": 0.1},
    net_config={"num_layers": 3, "ch": 128, "group": O2eR3()},
    log_level="info",
    trainer_config={
        "log_dir": None,
    ...
    if net_config["group"] is None:
        net_config["group"] = base_ds.symmetry
...
if __name__ == "__main__":
    ...
    cfg, outcome = Trial(
        argupdated_config(makeTrainer.__kwdefaults__, namespace=(emlp.groups, emlp.nn))
    )
    print(outcome)
