...
class HamiltonianDataset(Dataset):
    ...
    def __init__(
        self, n_systems=100, chunk_len=5, dt=0.2, integration_time=30, regen=False
    ):
        super().__init__()
        root_dir = os.path.expanduser(f"~/datasets/ODEDynamics/{self.__class__}/")
        filename = os.path.join(
            root_dir, f"trajectories_{n_systems}_{chunk_len}_{dt}_{integration_time}.pz"
        )
...
class DoubleSpringPendulum(HamiltonianDataset):
    ...
    def H(self, z):
        ...
        p1, p2 = unpack(p)
        x1, x2 = unpack(x)
        ke = 0.5 * (p1**2).sum(-1) / m1 + 0.5 * (p2**2).sum(-1) / m2
        pe = 0.5 * k1 * (jnp.sqrt((x1**2).sum(-1)) - l1) ** 2
        pe += k2 * (jnp.sqrt(((x1 - x2) ** 2).sum(-1)) - l2) ** 2
        pe += m1 * g * x1[..., 2] + m2 * g * x2[..., 2]
        return (ke + pe).sum()

    def sample_initial_conditions(self, bs):
        x1 = np.array([0, 0, -1.5]) + 0.2 * np.random.randn(bs, 3)
        x2 = np.array([0, 0, -3.0]) + 0.2 * np.random.randn(bs, 3)
        p = 0.4 * np.random.randn(bs, 6)
...
class IntegratedODETrainer(Regressor):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        self.gradvals = objax.Jit(
            objax.GradValues(self.loss, model.vars())
    ...
    def logStuff(self, step, minibatch=None):
        ...
        metrics = {
            "test_Rollout": np.exp(
                self.evalAverageMetrics(
                    loader, partial(log_rollout_error_ode, loader.dataset, self.model)
                )
            )
        }
        self.logger.add_scalars("metrics", metrics, step)
...
class Animation(object):
    def __init__(self, qt, lims=None, traj_lw=1, figkwargs={}):
        ...
        self.ax = (
            self.fig.add_axes([0, 0, 1, 1], projection="3d")
            if d == 3
            else self.fig.add_axes([0, 0, 1, 1])
        )
        ...
        delta = xyzmax - xyzmin
        lower = xyzmin - 0.1 * delta
        upper = xyzmax + 0.1 * delta
        if lims is None:
            lims = (
                (min(lower), max(upper)),
                (min(lower), max(upper)),
        ...
        self.colors = np.random.choice(
            [f"C{i}" for i in range(10)], size=n, replace=False
        )
        self.objects = {
            "pts": sum(
                [
                    self.ax.plot(*empty, "o", ms=6, color=self.colors[i])
                    for i in range(n)
                ],
                [],
            ),
            "traj_lines": sum(
                [
                    self.ax.plot(*empty, "-", color=self.colors[i], lw=traj_lw)
                    for i in range(n)
                ],
                [],
            ),
    ...
    def update(self, i=0):
        ...
        for j in range(n):
            ...
            if d == 3:
                self.objects["traj_lines"][j].set_3d_properties(xyz[..., 2].T)
            self.objects["pts"][j].set_data(*xyz[-1:, ..., :2].T)
            if d == 3:
                self.objects["pts"][j].set_3d_properties(xyz[-1:, ..., 2].T)
...
class PendulumAnimation(Animation):
    def __init__(self, qt, *args, **kwargs):
        ...
        self.objects["pts"] = sum(
            [
                self.ax.plot(*empty, "o", ms=10, c=self.colors[i])
                for i in range(self.qt.shape[1])
            ],
            [],
        )
...
def helix(Ns=1000, radius=0.05, turns=25):
...
class CoupledPendulumAnimation(PendulumAnimation):
    def __init__(self, *args, spring_lw=0.6, spring_r=0.2, **kwargs):
        super().__init__(*args, **kwargs)
        empty = self.qt.shape[-1] * [[]]
    ...
    def update(self, i=0):
        ...
        x, y, z = (
            (align2ref(diffs, self.helix) + qt_padded[:-1][:, None]).reshape(-1, 3).T
        )
    ...
    def __call__(self, cfg, i=None):
        try:
            ...
            for mb in trainer.dataloaders["test"]:
                trajectories.append(
                    pred_and_gt(trainer.dataloaders["test"].dataset, trainer.model, mb)
                )
            torch.save(
    ...
    def __call__(self, cfg, i=None):
        try:
            ...
            for mb in trainer.dataloaders["test"]:
                trajectories.append(
                    pred_and_gt_ode(
                        trainer.dataloaders["test"].dataset, trainer.model, mb
                    )
                )
            torch.save(
                np.concatenate(trajectories),
