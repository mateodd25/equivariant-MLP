...
class Rep(object):
    ...
    def constraint_matrix(self):
        ...
        constraints = []
        constraints.extend(
            [lazify(self.rho(h)) - I(n) for h in self.G.discrete_generators]
        )
    ...
    def equivariant_basis(self):
        ...
        if canon_rep not in self.solcache:
            ...
            logging.info(
                f"Solving basis for {self}"
                + (f", for G={self.G}" if hasattr(self, "G") else "")
            )
...
class Dual(Rep):
    ...
    def rho(self, M):
        rho = self.rep.rho(M)
        rhoinvT = (
            rho.invT() if isinstance(rho, LinearOperator) else jnp.linalg.inv(rho).T
        )
        return rhoinvT
...
V = (
    Vector
) = Base()  #: Alias V or Vector for an instance of the Base representation of a group
...
def sparsify_basis(Q, lr=1e-2):  # (n,r)
    ...
    opt_init, opt_update = optax.adam(
        lr
    )  # optax.sgd(1e2,.9)#optax.adam(lr)#optax.sgd(3e-3,.9)#optax.adam(lr)
    ...
    def loss(W):
        return (
            jnp.abs(Q @ W.T).mean()
            + 0.1 * (jnp.abs(W.T @ W - jnp.eye(W.shape[0]))).mean()
            + 0.01 * jax.numpy.linalg.slogdet(W)[1] ** 2
        )
    ...
    if (
        len(np.unique(np.abs(A))) != Q.shape[-1] + 1
        and len(np.unique(np.abs(A))) != Q.shape[-1]
    ):
        logging.error(
            f"Basis elems did not separate: found only {len(np.unique(np.abs(A)))}/{Q.shape[-1]}"
        )
...
def bilinear_weights(out_rep, in_rep):
    ...
    def lazy_projection(
        params, x
    ):  # (r,), (*c) #TODO: find out why backwards of this function is so slow
        bshape = x.shape[:-1]
