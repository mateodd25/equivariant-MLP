...
class SumRep(Rep):
    ...
    def rho(self, M):
        ...
        return (
            LazyPerm(self.invperm)
            @ LazyDirectSum(rhos, multiplicities)
            @ LazyPerm(self.perm)
        )
    ...
    def drho(self, A):
        ...
        return (
            LazyPerm(self.invperm)
            @ LazyDirectSum(drhos, multiplicities)
            @ LazyPerm(self.perm)
        )
    ...
    def equivariant_basis(self):
        ...
        Qs = {
            rep: (
                jax.device_put(Q.astype(np.float32))
        ...
        return LinearOperator(
            shape=(self.size(), active_dims), matvec=lazy_Q, matmat=lazy_Q
        )
    ...
    def equivariant_projector(self):
        ...
        def lazy_P(array):
            return lazy_direct_matmat(array[self.perm], Ps.values(), multiplicities)[
                self.invperm
            ]  # [:,self.invperm]

        return LinearOperator(
            shape=(self.size(), self.size()), matvec=lazy_P, matmat=lazy_P
        )
    ...
    def __iter__(
        self,
    ):  # not a great idea to use this method (ignores permutation ordering)
...
class DirectProduct(ProductRep):
    ...
    def equivariant_projector(self):
        canon_P = LazyKron(
            [rep.equivariant_projector() for rep, c in self.reps.items()]
        )
