...
class Inertia(object):
    def __init__(self, N=1024, k=5):
        ...
        Xstd[k:] = (
            np.abs(self.X[:, k:].reshape(N, k, 3)).mean((0, 2))[:, None]
            + np.zeros((k, 3))
        ).reshape(k * 3)
...
class ParticleInteraction(object):
    ...
    def __init__(self, N=1024):
        ...
        Le = (
            p1[:, :, None] * p3[:, None, :]
            - (dot(p1, p3) - dot(p1, p1))[:, None, None] * 𝜂
        )
        ...
        self.Xscale = np.sqrt(
            (
                np.abs((self.X.reshape(N, 4, 4) @ 𝜂) * self.X.reshape(N, 4, 4)).mean(-1)
            ).mean(0)
        )
        ...
        self.stats = (
            ...
            self.Y.mean(axis=0),
            self.Y.std(axis=0),
        )  # self.X.mean(axis=0),self.X.std(axis=0),
...
class GroupAugmentation(Module):
    ...
    def __call__(self, x, training=True):
        if training:
            ...
            return (
                rhout_inv
                @ self.model((self.rho_in(gs) @ x[..., None])[..., 0], training)[
                    ..., None
                ]
            )[..., 0]
        else:
...
def ULBcorner_rot(state, i=1):
    ...
    ULB_corner_ids = np.array(
        [0, 4 * 8, 3 * 8 + 2]
    )  # top left of U, top left of L, top right of B
    rotated_corner_state = state.copy()
    rotated_corner_state[:, ULB_corner_ids] = rotated_corner_state[
        :, np.roll(ULB_corner_ids, i)
    ]
...
def LBface_swap(state):
    ...
    piece_swap[:, L_B_center_top_faces] = piece_swap[
        :, np.roll(L_B_center_top_faces, 1)
    ]
    return piece_swap
...
class BrokenRubiksCube(object):
    ...
    def __init__(self, train=True):
        ...
        transforms = [
            Id,
            itertools.product(
                [Id, ULBcorner_rot, partial(ULBcorner_rot, i=2)],
                [Id, UBedge_flip],
                [Id, LBface_swap],
            ),
        ]
        ...
        parity_perm = np.array([5, 3, 4, 1, 2, 0])
        self.X[11:] = solved_state.reshape(6, 6, 8)[:, parity_perm].reshape(
            -1
