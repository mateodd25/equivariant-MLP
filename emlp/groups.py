...
from emlp.reps.linear_operators import (
    LazyShift,
    SwapMatrix,
    Rot90,
    LazyKron,
...
def rel_err(A, B):
    return jnp.mean(jnp.abs(A - B)) / (
        jnp.mean(jnp.abs(A)) + jnp.mean(jnp.abs(B)) + 1e-6
    )
...
class Group(object, metaclass=Named):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.args = args
        if isinstance(self.lie_algebra, np.ndarray):
            self.lie_algebra = jax.device_put(self.lie_algebra)
        if isinstance(self.discrete_generators, np.ndarray):
        ...
        if self.is_orthogonal is None:
            ...
            if len(self.discrete_generators) != 0:
                h_dense = jnp.stack(
                    [hi @ jnp.eye(self.d) for hi in self.discrete_generators]
                )
                self.is_orthogonal &= (
                    rel_err(h_dense.transpose((0, 2, 1)) @ h_dense, jnp.eye(self.d))
                    < 1e-6
                )
    ...
    def samples(self, N):
        ...
        A_dense = (
            jnp.stack([Ai @ jnp.eye(self.d) for Ai in self.lie_algebra])
            if len(self.lie_algebra)
            else jnp.zeros((0, self.d, self.d))
        )
        h_dense = (
            jnp.stack([hi @ jnp.eye(self.d) for hi in self.discrete_generators])
...
def noise2sample(z, ks, lie_algebra, discrete_generators, seed=0):
    ...
    for k in range(K):  # multiple rounds of discrete generators
        ...
        for i in jax.random.permutation(pkey, M):  # Randomize the order of generators
            g = g @ matrix_power_simple(
                discrete_generators[i], ks[i, k]
            )  # jnp.linalg.matrix_power(discrete_generators[i],ks[i])
    return g


@jit
def noise2samples(zs, ks, lie_algebra, discrete_generators, seed=0):
    return vmap(noise2sample, (0, 0, None, None, None), 0)(
        zs, ks, lie_algebra, discrete_generators, seed
    )
...
class SO13p(Group):
    ...
    z_scale = np.array([0.3, 0.3, 0.3, 1, 1, 1])  # can get rid of now
...
class RubiksCube(Group):  # 3x3 rubiks cube
    ...
    def __init__(self):
        ...
        order_padded[0, :] = np.rot90(order_padded[0].reshape(3, 3), 1).reshape(
            9
        )  # Rotate top face
        ...
        RotFront = pad(
            np.arange(48)
        )  # rotate full cube so that Left face becomes Up, Up becomes Right, Right becomes Down, Down becomes Left
        ...
        Dperm = RotRight[
            RotRight[Uperm[RotLeft[RotLeft]]]
        ]  # Dperm = RotLeft<-RotLeft<-Uperm<-RotRight<-RotRight
        self.discrete_generators = [
            LazyPerm(perm) for perm in [Uperm, Fperm, Rperm, Bperm, Lperm, Dperm]
        ]
        super().__init__()
...
class DirectProduct(Group):
    def __init__(self, G1, G2):
        ...
        self.discrete_generators = [
            LazyKron([M1, I2]) for M1 in G1.discrete_generators
