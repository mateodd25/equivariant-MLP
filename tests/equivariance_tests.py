...
def test_high_rank_representations(G):
    ...
    for p in range(r + 1):
        for q in range(r - p + 1):
            ...
            assert (
                err < 1e-4
            ), f"Symmetric vector fails err {err:.3e} with T{p,q} and G={G}"
...
@parametrize(
    [
        ...
        (
            SO13p(),
            T(2) + 4 * T(1, 0) + T(0, 1),
            10 * T(0) + 3 * T(1, 0) + 3 * T(0, 1) + T(0, 2) + T(2, 0) + T(1, 1),
        ),
        (Sp(2), (V + 2 * V**2) * (V.T + 1 * V).T + V.T, 3 * V**0 + V + V * V.T),
        (
            SU(3),
            T(2, 0) + T(1, 1) + T(0) + 2 * T(0, 1),
            T(1, 1) + V + V.T + T(0) + T(2, 0) + T(0, 2),
        ),
    ]
)
def test_equivariant_matrix(G, repin, repout):
    ...
    gWerr = vmap(scale_adjusted_rel_error)(
        gvecW, W.reshape(-1) + jnp.zeros_like(gvecW), gs
    ).mean()
...
def test_bilinear_layer(G, repin, repout):
    ...
    assert (
        equiv_err < 1e-4
    ), f"Bilinear Equivariance fails err {equiv_err:.3e} with G={G}"
...
def test_large_representations(G):
    ...
    assert (
        equiv_err < 1e-4
    ), f"Large Rep Equivariant gWx=Wgx fails err {equiv_err:.3e} with G={G}"
