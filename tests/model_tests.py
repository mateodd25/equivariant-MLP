...
def get_dsmb(dsclass):
    ...
    dataloader = DataLoader(
        ds, batch_size=min(bs, len(ds)), num_workers=0, pin_memory=False
    )
...
def test_init_forward_and_equivariance(dsclass):
    ...
    assert (
        equivariance_err(model, mb, ds.rep_in, ds.rep_out, ds.symmetry) < 1e-4
    ), "Objax EMLP failed equivariance test"
...
def test_haiku_emlp(dsclass):
    ...
    assert (
        equivariance_err(model, mb, ds.rep_in, ds.rep_out, ds.symmetry) < 1e-4
    ), "Haiku EMLP failed equivariance test"
...
def test_flax_emlp(dsclass):
    ...
    assert (
        equivariance_err(model, mb, ds.rep_in, ds.rep_out, ds.symmetry) < 1e-4
    ), "flax EMLP failed equivariance test"
...
def test_pytorch_emlp(dsclass):
    ...
    model = lambda x: jax.device_put(
        net(torch.from_numpy(np.asarray(x)).to(device)).cpu().data.numpy()
    )
    assert (
        equivariance_err(model, mb, ds.rep_in, ds.rep_out, ds.symmetry) < 1e-4
    ), "Pytorch EMLP failed equivariance test"
...
def test_bespoke_representations():
    ...
    assert (
        rel_error(out1, out2) < 1e-4
    ), "EMLP equivariance fails on bespoke productsubrep"
